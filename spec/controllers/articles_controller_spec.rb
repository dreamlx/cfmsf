...
describe Admin::ArticlesController do
  ...
  before :each do 
    # @news = create(:news)
    # @life = create(:life)
    @news = Category.create(chinese_name: "新闻", french_name: "news")
    @life = Category.create(chinese_name: "生活", french_name: "life")
  ...
  describe "PATCH #update" do
    it "update the new article into database" do
      put :update, id: @news_article01, article: attributes_for(:article, category_id: @news) 
      @news_article01.reload
      # expect(@news_article01.chinese_title).to eq @news_article01.chinese_title
      ActionMailer::Base.deliveries.count.should == 1
      ActionMailer::Base.deliveries.first.from.should == ['notifications@example.com']
      ActionMailer::Base.deliveries.first.to.should == [User.where(:role => 'admin').first.email]
      ActionMailer::Base.deliveries.first.subject.should == "Article '#{@news_article01.chinese_title}' was updated"
    end
  end
end